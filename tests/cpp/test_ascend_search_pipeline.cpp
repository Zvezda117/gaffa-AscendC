...
bool by_peak_location(const gaffa::AscendFfaBatchPeak& lhs,
                      const gaffa::AscendFfaBatchPeak& rhs) {
...
TEST(AscendSearchPipeline, DeviceDedispersionPreprocessAndFfaMatchCpu) {
  ...
  auto ascend_dedispersed = gaffa::dedisperse_subband_ascend_device(
      input, frequency_mhz, dedispersion_plan, subband_options);
  ASSERT_EQ(ascend_dedispersed.shape.ndm, ndm);
  ASSERT_EQ(ascend_dedispersed.shape.nsamples, nsamples);

  gaffa::AscendDeviceBuffer<float> device_series(
      ascend_dedispersed.size(), ascend_dedispersed.device_id);
  gaffa::convert_time_series_batch_to_float_ascend(
      {.data = ascend_dedispersed.data.data(),
       .count = ascend_dedispersed.size(),
       .device_id = ascend_dedispersed.device_id},
      ndm, nsamples,
      device_series.as_span());

  gaffa::AscendPreprocessProgram preprocess_program(
      preprocess_plan,
      {.device_id = ascend_dedispersed.device_id},
      {.series_tile_size = ndm, .max_nsamples = nsamples});
  const gaffa::MutableAscendTimeSeriesBatchView mutable_batch{
      .data = device_series.data(),
      .nseries = ndm,
      .nsamples = nsamples,
      .device_id = ascend_dedispersed.device_id,
  };
  gaffa::preprocess_time_series_batch_inplace_ascend(
  ...
  auto actual = gaffa::run_ffa_batch_ascend(
                    ffa_program, mutable_batch.as_const(), search_options)
                    .peaks;
